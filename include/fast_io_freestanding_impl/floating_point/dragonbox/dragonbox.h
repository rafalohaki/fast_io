...
namespace fast_io::jkj::dragonbox {
namespace detail {
template <class T>
...
struct default_ieee754_traits {
	...
	constexpr
#endif
	T carrier_to_float(carrier_uint u) noexcept {
#if __cpp_lib_bit_cast >= 201806L
	...
	constexpr
#endif
	carrier_uint float_to_carrier(T x) noexcept {
#if __cpp_lib_bit_cast >= 201806L
...
struct ieee754_bits {
	...
	constexpr ieee754_bits() = default;
...
constexpr bool divisible_by_power_of_2(UInt x, unsigned int exp) noexcept {
	return std::countr_zero(x) >= int(exp);
}
...
[[msvc::forceinline]] __declspec(safebuffers)
#endif
static inline void shorter_interval_case(
...
using policy_holder = decltype(make_policy_holder(
	base_default_pair_list<
		...
		base_default_pair<cache::base, cache::normal>>{}, policies...));
