...
public:
	...
	explicit cs_insn_range(cs_io_observer csiob,char const* buffer,std::size_t code_size,std::uint64_t address,std::size_t count) noexcept
	{
		count=noexcept_call(cs_disasm,csiob.handle,reinterpret_cast<std::uint8_t const*>(code),code_size,address,count,__builtin_addressof(ins));
	}
