...
public:
	using char_type = T;
	using native_handle_type = CFile*;
	explicit constexpr basic_mfc_io_handle() noexcept=default;
	explicit constexpr basic_mfc_io_handle(native_handle_type hd) noexcept:basic_mfc_io_observer<T>{hd}{}
	basic_mfc_io_handle(basic_mfc_io_handle const& mcf):basic_mfc_io_observer<T>{mcf.phandle->Duplicate()}{}
	...
	basic_mfc_io_handle(basic_mfc_io_handle&& mcf) noexcept:basic_mfc_io_observer<T>{mcf.phandle}
	{
		mcf.phandle=nullptr;
	}
...
class basic_mfc_file:public basic_mfc_io_handle<ch_type>
{
public:
	...
	explicit constexpr basic_mfc_file(T hd) noexcept:basic_mfc_io_handle<ch_type>(hd){}
	template<win32_family family>
	explicit basic_mfc_file(basic_win32_family_io_handle<family,char_type>&& hd,open_mode):
		basic_mfc_io_handle<char_type>(new CFile(hd.handle))
	{
		hd.release();
	}
	template<nt_family family>
	explicit basic_mfc_file(basic_nt_family_io_handle<family,char_type>&& hd,open_mode):
		basic_mfc_io_handle<char_type>(new CFile(hd.handle))
	{
		hd.release();
	}
...
using wmfc_io_handle=basic_mfc_io_handle<wchar_t>;
using wmfc_file=basic_mfc_file<wchar_t>;
using u16mfc_io_observer=basic_mfc_io_observer<char16_t>;
using u16mfc_io_handle=basic_mfc_io_handle<char16_t>;
using u16mfc_file=basic_mfc_file<char16_t>;
using u32mfc_io_observer=basic_mfc_io_observer<char32_t>;
using u32mfc_io_handle=basic_mfc_io_handle<char32_t>;
using u323mfc_file=basic_mfc_file<char32_t>;
