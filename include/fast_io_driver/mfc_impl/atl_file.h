...
template<std::integral T>
class basic_atl_io_handle:public basic_atl_io_observer<T>
{
public:
	using char_type = T;
	using native_handle_type = CAtlFile*;
	explicit constexpr basic_atl_io_handle() noexcept=default;
	explicit constexpr basic_atl_io_handle(native_handle_type hd) noexcept:basic_atl_io_observer<T>{hd}{}
	basic_atl_io_handle(basic_atl_io_handle const& mcf):basic_atl_io_observer<T>{mcf.phandle->Duplicate()}{}
	basic_atl_io_handle& operator=(basic_atl_io_handle const& mcf)
	{
	...
	basic_atl_io_handle(basic_atl_io_handle&& mcf) noexcept:basic_atl_io_observer<T>{mcf.phandle}
	{
		mcf.phandle=nullptr;
	}
...
class basic_atl_file:public basic_atl_io_handle<ch_type>
{
public:
	...
	explicit constexpr basic_atl_file(T hd) noexcept:basic_atl_io_handle<ch_type>(hd){}
	template<win32_family family>
	explicit basic_atl_file(basic_win32_family_io_handle<family,char_type>&& hd,open_mode):
		basic_atl_io_handle<char_type>(new CAtlFile(hd.handle))
	{
		hd.release();
	}
	template<nt_family family>
	explicit basic_atl_file(basic_nt_family_io_handle<family,char_type>&& hd,open_mode):
		basic_atl_io_handle<char_type>(new CAtlFile(hd.handle))
	{
		hd.release();
	}
	...
	{}
	basic_atl_file(basic_atl_file const&)=default;
	basic_atl_file& operator=(basic_atl_file const&)=default;
	basic_atl_file(basic_atl_file&&) noexcept=default;
	basic_atl_file& operator=(basic_atl_file&&) noexcept=default;
	~basic_atl_file()
	{
		delete this->phandle;
	}
