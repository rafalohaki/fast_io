...
inline void linux_kpr_raw_write_fmt(char const* fmt,void const* start,void const* last) noexcept
{
	...
	::fast_io::linux::linux_kernel_printk(fmt,static_cast<int>(diff),start_char_ptr);
}
...
inline void deal_with_kpr_ignore_line(char const* fmt_str,Args ...args)
{
	...
	if constexpr(n==0)
	{
		::fast_io::linux::linux_kernel_printk(fmt_str);
	}
	else
	{
...
{
	if constexpr((::fast_io::linux::details::kpr_total_print_reserve_size_less_or_equal_than512<ch_type,Args...>(0)))
		::fast_io::linux::details::deal_with_kpr_common<line>(k,args...);
	else
