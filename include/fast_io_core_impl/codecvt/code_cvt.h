...
template<std::integral src_char_type,
	encoding_scheme src_scheme=details::execution_charset_encoding_scheme<char_type>(),
	encoding_scheme dst_scheme=encoding_scheme::execution_charset>
struct basic_code_converter
{
	using char_type=ftype;
	using from_scheme = src_scheme;
	using to_scheme = dst_scheme;
	basic_mb_state<from_char_type,to_scheme> state;
};
...
using u32code_converter=basic_code_converter<char16_t>;
...
inline constexpr Iter deco_reserve_define(io_reserve_type_t<std::iter_value_t<Iter>,
basic_code_converter<std::iter_value_t<from_iter>,from_scheme,to_scheme>>,
basic_code_converter<std::iter_value_t<from_iter>,from_scheme,to_scheme>& conv,from_iter src_frst,from_iter src_last,Iter dst) noexcept
{
	using from_char_type = std::iter_value_t<from_iter>;
	using to_char_type = std::iter_value_t<to_iter>;
	if constexpr(std::is_pointer_v<to_iter>)
		return details::general_code_cvt<from_scheme,to_scheme>(conv.state,
			std::to_address(src_first),
			std::to_address(src_last),
			dst);
	else
	{
		return dst+(details::general_code_cvt<from_scheme,to_scheme>(conv.state,
		std::to_address(src_first),
		std::to_address(src_last),
