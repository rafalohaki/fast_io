...
#if 0
enum class http_line_stage:char8_t
{
none,first_crlf,second_crlf,content
};
#endif
...
struct basic_http_line_buffer
{
	...
	std::size_t pos{};
#if 0
	http_line_stage stage{http_line_stage::none};
#endif
};
...
template<typename State,::fast_io::freestanding::random_access_iterator Iter>
inline constexpr parse_result<Iter> scan_context_define_http_header_line_impl(State state,Iter first,Iter last,basic_http_header_line<::fast_io::freestanding::iter_value_t<Iter>>& line) noexcept
{
	std::size_t const diff{static_cast<std::size_t>(last-first)};
	std::size_t const total_size{static_cast<std::size_t>(last-first)+state.pos};
	if(total_size<4u)
	{
		non_overlapped_copy_n(first,diff,state.buffer+state.pos);
		state.pos+=diff;
		return {last,parse_code::partial};
	}
	if(state.pos<4u)
	{
		std::size_t const df2{};
		non_overlapped_copy_n(first,last,state.pos);
	}
	using char_type = ::fast_io::freestanding::iter_value_t<Iter>;
	...
	case http_line_stage::none:
	{
		if(*first==sign_ch<u8'\r',char_type>)
		{
			...
			if(first==last)
			{
				state.stage=http_line_stage::first_crlf;
				return {first,parse_code::none};
			}
		}	
		[[fallthrough]];
	}
	case http_line_stage::first_crlf:
	{
		if(*first==sign_ch<u8'\n',char_type>)
		{
			++first;
			state.stage=http_line_stage::second_crlf;
			if(first==last)
			{
		...
		{
			state.stage=http_line_stage::content;
		}
		[[fallthrough]];
	}
	case http_line_stage::second_crlf:
	{
